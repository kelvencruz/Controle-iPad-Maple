...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .grid { display: grid; grid-template-columns: repeat(3, 1fr); padding: 10mm; gap: 0; }
    .card { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 6mm 4mm; border: 0.5px solid #ccc; page-break-inside: avoid; break-inside: avoid; }
    .qr { display: block; line-height: 0; }
    .qr svg { display: block; }
    .name { margin-top: 3mm; font-size: 9pt; font-weight: 600; text-align: center; word-break: break-word; max-width: 120px; line-height: 1.3; }
    .serial { margin-top: 1mm; font-size: 7pt; color: #555; text-align: center; font-family: monospace; }
    @media print { html, body { width: 210mm; } .grid { padding: 10mm; } }
  </style>
</head>
<body>
  ...
  if (!win) {
    toast.error("Permita pop-ups para esta página para usar a impressão.");
    return;
  }
  win.document.open();
        ...
        <div className="mt-8 rounded-2xl border border-[var(--color-error-container)] bg-[var(--color-error-container)] text-[var(--color-on-error-container)] px-5 py-4 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium">
              {networkError ? "Sem conexão com a internet" : "Erro ao carregar dispositivos"}
            </p>
    ...
    return (
      <main className="px-4 md:px-6 pb-24 md:pb-6">
        <div className="mt-16 flex flex-col items-center gap-2 text-[var(--color-on-surface-variant)]">
          ...
          <p className="text-sm">Cadastre iPads em Dispositivos para gerar QR codes.</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="px-4 md:px-6 pb-24 md:pb-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-2 mb-5">
        <div>
          <h1 className="text-xl font-semibold text-[var(--color-on-surface)]">QR Codes</h1>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-medium border border-[var(--color-outline-variant)] bg-[var(--color-surface-container)] text-[var(--color-on-surface)] hover:bg-[var(--color-surface-container-high)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            Imprimir selecionados
          </button>
          <button
            onClick={handlePrintAll}
            className="inline-flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-medium bg-[var(--color-primary)] text-[var(--color-on-primary)] hover:opacity-90 transition-opacity"
          >
            Imprimir todos
          </button>
        </div>
        ...
        >
          {allSelected && (
            <svg viewBox="0 0 12 12" className="w-3 h-3" fill="none">
              <path d="M2 6l3 3 5-5" stroke="var(--color-on-primary)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          )}
          {someSelected && !allSelected && (
            <svg viewBox="0 0 12 12" className="w-3 h-3" fill="none">
              <path d="M3 6h6" stroke="var(--color-primary)" strokeWidth="1.8" strokeLinecap="round" />
            </svg>
          )}
        </button>
        ...
        {devices.map((device) => {
          ...
          return (
            <button
              ...
              className="relative flex flex-col items-center gap-2 rounded-2xl p-4 text-left border-2 transition-all cursor-pointer hover:bg-[var(--color-surface-container-high)]"
              style={{
                borderColor: isSelected ? "var(--color-primary)" : "var(--color-outline-variant)",
                backgroundColor: isSelected ? "var(--color-primary-container)" : "var(--color-surface-container)",
              }}
            ...
            >
              ...
              <div className="rounded-xl p-2 bg-white" style={{ lineHeight: 0 }}>
                <QRCodeSVG value={device.qr_code} size={80} level="M" includeMargin={false} />
              </div>
